...
macro_rules! check {
    ($cond:expr, $err:expr) => {
        if !($cond) {
            let error_code: $crate::errors::SurroundfiError = $err;
...
macro_rules! check_eq {
    ($left:expr, $right:expr, $err:expr) => {
        if $left != $right {
            let err_code: $crate::errors::SurroundfiError = $err;
            #[cfg(not(feature = "test-bpf"))]
            anchor_lang::prelude::msg!(
...
macro_rules! math_error {
    () => {{
        || {
            let error_code = $crate::errors::SurroundfiError::MathError;
            anchor_lang::prelude::msg!(
                "Error \"{}\" thrown at {}:{}",
                error_code,
                file!(),
...
// temporary disable live check for testing
#[macro_export]
macro_rules! live {
    () => {
        true
