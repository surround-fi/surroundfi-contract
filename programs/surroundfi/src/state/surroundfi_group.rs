use super::{
    surroundfi_account::{BalanceSide, RequirementType},
...
use crate::{
    ...
    set_if_some,
    state::surroundfi_account::calc_value,
    SurroundfiResult,
};
use crate::{
...
assert_struct_size!(SurroundfiGroup, 1056);
#[account(zero_copy)]
#[derive(Default, Debug, PartialEq, Eq, TypeLayout)]
pub struct SurroundfiGroup {
    pub admin: Pubkey,
...
impl SurroundfiGroup {
    ...
    // nothing.
    pub fn add_bank(&mut self) -> SurroundfiResult {
...
impl InterestRateConfig {
    pub fn create_interest_rate_calculator(&self, group: &SurroundfiGroup) -> InterestRateCalc {
    ...
    pub fn validate(&self) -> SurroundfiResult {
        ...
        check!(
            ...
            SurroundfiError::InvalidConfig
        );
        check!(plateau_ir > I80F48::ZERO, SurroundfiError::InvalidConfig);
        check!(max_ir > I80F48::ZERO, SurroundfiError::InvalidConfig);
        check!(plateau_ir < max_ir, SurroundfiError::InvalidConfig);
...
impl Bank {
    ...
    pub fn get_liability_amount(&self, shares: I80F48) -> SurroundfiResult<I80F48> {
        Ok(shares
            .checked_mul(self.liability_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_asset_amount(&self, shares: I80F48) -> SurroundfiResult<I80F48> {
        Ok(shares
            .checked_mul(self.asset_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_liability_shares(&self, value: I80F48) -> SurroundfiResult<I80F48> {
        Ok(value
            .checked_div(self.liability_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_asset_shares(&self, value: I80F48) -> SurroundfiResult<I80F48> {
        Ok(value
    ...
    pub fn change_asset_shares(
        ...
        if shares.is_positive() && self.config.is_deposit_limit_active() && !bypass_deposit_limit {
            ...
            check!(
                ...
                crate::prelude::SurroundfiError::BankAssetCapacityExceeded
            )
        }
    ...
    pub fn maybe_get_asset_weight_init_discount(
        &self,
        price: I80F48,
    ) -> SurroundfiResult<Option<I80F48>> {
    ...
    pub fn change_liability_shares(
        ...
        if bypass_borrow_limit.not() && shares.is_positive() && self.config.is_borrow_limit_active()
        {
            ...
            check!(
                ...
                crate::prelude::SurroundfiError::BankLiabilityCapacityExceeded
            )
        }
    ...
    pub fn check_utilization_ratio(&self) -> SurroundfiResult {
        ...
        check!(
            ...
            crate::prelude::SurroundfiError::IllegalUtilizationRatio
        );

        Ok(())
    }

    pub fn configure(&mut self, config: &BankConfigOpt) -> SurroundfiResult {
    ...
    pub fn configure_unfrozen_fields_only(&mut self, config: &BankConfigOpt) -> SurroundfiResult {
        set_if_some!(self.config.deposit_limit, config.deposit_limit);
        set_if_some!(self.config.borrow_limit, config.borrow_limit);
    ...
    pub fn deposit_spl_transfer<'info>(
        ...
        check!(
            ...
            SurroundfiError::InvalidTransfer
        );
    ...
    pub fn socialize_loss(&mut self, loss_amount: I80F48) -> SurroundfiResult {
        let total_asset_shares: I80F48 = self.total_asset_shares.into();
    ...
    pub fn assert_operational_mode(
        ...
        match self.config.operational_state {
            BankOperationalState::Paused => Err(SurroundfiError::BankPaused.into()),
            ...
            BankOperationalState::ReduceOnly => {
                    ...
                    is_asset_or_liability_amount_increasing
                {
                    check!(
                        !is_asset_or_liability_amount_increasing,
                        SurroundfiError::BankReduceOnly
                    );
                }
...
impl BankConfig {
    ...
    pub fn validate(&self) -> SurroundfiResult {
        ...
        check!(asset_maint_w >= asset_init_w, SurroundfiError::InvalidConfig);

        let liab_init_w = I80F48::from(self.liability_weight_init);
        let liab_maint_w = I80F48::from(self.liability_weight_maint);

        check!(liab_init_w >= I80F48::ONE, SurroundfiError::InvalidConfig);
        check!(
        ...
        if self.risk_tier == RiskTier::Isolated {
            check!(asset_init_w == I80F48::ZERO, SurroundfiError::InvalidConfig);
            check!(asset_maint_w == I80F48::ZERO, SurroundfiError::InvalidConfig);
        }
    ...
    pub fn validate_oracle_age(&self) -> SurroundfiResult {
        check!(
            self.oracle_max_age >= ORACLE_MIN_AGE,
            SurroundfiError::InvalidOracleSetup
        );
        Ok(())
    }
...
mod tests {
    ...
    fn ir_config_calc_interest_rate_pff_01() {
        ...
        let ComputedInterestRates {
            ...
            .create_interest_rate_calculator(&SurroundfiGroup::default())
    ...
    fn interest_rate_accrual_test_0() -> anyhow::Result<()> {
        ...
        let mut group = SurroundfiGroup::default();
        group.group_flags = 1;
