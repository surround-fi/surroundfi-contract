...
use crate::{assert_struct_align, assert_struct_size, check, SurroundfiError, SurroundfiResult};

use super::surroundfi_group::{RiskTier, WrappedI80F48};
...
pub struct StakedSettings {
    /// This account's own key. A PDA derived from `surroundfi_group` and `STAKED_SETTINGS_SEED`
    pub key: Pubkey,
...
impl StakedSettings {
    ...
    pub fn initialize(&mut self, surroundfi_group: Pubkey) {
        self.surroundfi_group = surroundfi_group;
    }
...
impl Default for StakedSettings {
    fn default() -> Self {
        StakedSettings {
            key: Pubkey::default(),
            surroundfi_group: Pubkey::default(),
