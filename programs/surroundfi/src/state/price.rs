...
use super::surroundfi_group::BankConfig;
...
impl OraclePriceFeedAdapter {
    ...
    pub fn try_from_bank_config_with_max_age<'info>(
        ...
        match bank_config.oracle_setup {
            OracleSetup::None => Err(SurroundfiError::OracleNotSetup.into()),
            OracleSetup::PythLegacy => {
                check!(ais.len() == 1, SurroundfiError::WrongNumberOfOracleAccounts);
                if ais[0].key != &bank_config.oracle_keys[0] {
                    ...
                    return Err(error!(SurroundfiError::WrongOracleAccountKeys));
                }
                ...
                if live!() {
                    check_eq!(
                        *account_info.owner,
                        ...
                        SurroundfiError::PythPushWrongAccountOwner
                    );
                } else {
                    ...
                    check!(owner_ok, SurroundfiError::PythPushWrongAccountOwner);
                }
            ...
            OracleSetup::StakedWithPythPush => {
                check!(ais.len() == 3, SurroundfiError::WrongNumberOfOracleAccounts);
                ...
                if cfg!(any(
                    ...
                    check_eq!(
                        account_info.owner,
                        &pyth_solana_receiver_sdk::id(),
                        SurroundfiError::StakedPythPushWrongAccountOwner
                    );
    ...
    pub fn validate_bank_config(
        ...
        match bank_config.oracle_setup {
            ...
            OracleSetup::PythLegacy => {
                check!(
                    oracle_ais.len() == 1,
                    SurroundfiError::WrongNumberOfOracleAccounts
                );
            ...
            OracleSetup::StakedWithPythPush => {
                if lst_mint.is_some() && stake_pool.is_some() && sol_pool.is_some() {
                    ...
                    check_eq!(exp_mint, lst_mint, SurroundfiError::StakePoolValidationFailed);
...
impl PythLegacyPriceFeed {
    pub fn load_checked(ai: &AccountInfo, current_time: i64, max_age: u64) -> SurroundfiResult<Self> {
        ...
        let ema_price = if live!() {
            price_feed
                ...
                .ok_or(SurroundfiError::InternalLogicError)?
        } else {
    ...
    fn check_ais(ai: &AccountInfo) -> SurroundfiResult {
        load_pyth_price_feed(ai)?;
        Ok(())
    }

    fn get_confidence_interval(&self, use_ema: bool) -> SurroundfiResult<I80F48> {
    ...
    #[inline(always)]
    fn get_ema_price(&self) -> SurroundfiResult<I80F48> {
        pyth_price_components_to_i80f48(I80F48::from_num(self.ema_price.price), self.ema_price.expo)
    }

    #[inline(always)]
    fn get_unweighted_price(&self) -> SurroundfiResult<I80F48> {
...
impl SwitchboardPullPriceFeed {
    ...
    fn get_price(&self) -> SurroundfiResult<I80F48> {
    ...
    fn get_confidence_interval(&self) -> SurroundfiResult<I80F48> {
...
impl SwitchboardV2PriceFeed {
    pub fn load_checked(
        ...
        check!(
            ...
            SurroundfiError::InternalLogicError
        );

        let aggregator_account = AggregatorAccountData::new_from_bytes(&ai_data)
            .map_err(|_| SurroundfiError::InternalLogicError)?;
    ...
    fn get_price(&self) -> SurroundfiResult<I80F48> {
        let sw_decimal = self
            .aggregator_account
            .get_result()
            .map_err(|_| SurroundfiError::InvalidPrice)?;

        Ok(switchboard_decimal_to_i80f48(sw_decimal)
            .ok_or(SurroundfiError::InvalidSwitchboardDecimalConversion)?)
    }
...
pub fn load_price_update_v2_checked(ai: &AccountInfo) -> SurroundfiResult<PriceUpdateV2> {
    if live!() {
        check_eq!(
            *ai.owner,
            pyth_solana_receiver_sdk::id(),
    ...
    check!(
        ...
        SurroundfiError::PythPushInvalidAccount
    );
...
impl PythPushOraclePriceFeed {
    ...
    /// There can be multiple pyth push oracles for a given feed_id. Surroundfi allows using any
    ...
    pub fn load_checked(
        ...
        let price = price_feed_account
            ...
            .map_err(|e| {
                ...
                let error: SurroundfiError = e.into();
                error
            })?;
        ...
        msg!("PythPushOracle - load_checked: price: {}, exponent: {}, publish_time: {}", price.price, price.exponent, price.publish_time);
    ...
    #[cfg(feature = "client")]
    pub fn load_unchecked(ai: &AccountInfo) -> SurroundfiResult<Self> {
        let price_feed_account = load_price_update_v2_checked(ai)?;
    ...
    #[cfg(feature = "client")]
    pub fn peek_feed_id(ai: &AccountInfo) -> SurroundfiResult<FeedId> {
        let price_feed_account = load_price_update_v2_checked(ai)?;

        Ok(price_feed_account.price_message.feed_id)
    }

    pub fn check_ai_and_feed_id(ai: &AccountInfo, feed_id: &FeedId) -> SurroundfiResult {
        let price_feed_account = load_price_update_v2_checked(ai)?;
    ...
    ///
    /// Surroundfi sponsored feed id
    /// `constants::PYTH_PUSH_SURROUNDFI_SPONSORED_SHARD_ID = 3301`
...
impl LiteAggregatorAccountData {
    ...
    pub fn get_result(&self) -> anchor_lang::Result<SwitchboardDecimal> {
        ...
        if min_oracle_results > latest_confirmed_round_num_success {
            return Err(SurroundfiError::SwitchboardInvalidAccount.into());
        }
...
#[inline(always)]
fn pyth_price_components_to_i80f48(price: I80F48, exponent: i32) -> SurroundfiResult<I80F48> {
...
fn load_pyth_price_feed(ai: &AccountInfo) -> SurroundfiResult<PriceFeed> {
    check!(ai.owner.eq(&PYTH_ID), SurroundfiError::InternalLogicError);
