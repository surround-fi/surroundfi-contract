...
async fn add_bank_success() -> anyhow::Result<()> {
    ...
    let fee_wallet = test_f.surroundfi_group.fee_wallet;
    ...
    for (mint_f, bank_config) in mints {
        ...
        let _ = {
            ...
            assert_eq!(group, test_f.surroundfi_group.key);
...
#[tokio::test]
async fn surroundfi_group_add_bank_failure_inexistent_pyth_feed() -> anyhow::Result<()> {
    // Setup test executor with non-admin payer
    let test_f = TestFixture::new(None).await;
    ...
    let res = test_f
        ...
        .try_lending_pool_add_bank(
            ...
            BankConfig {
                oracle_setup: surroundfi::state::price::OracleSetup::PythLegacy,
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::InternalLogicError);
...
async fn configure_bank_success(bank_mint: BankMint) -> anyhow::Result<()> {
    ...
    let config_bank_opt = BankConfigOpt {
        interest_rate_config: Some(surroundfi::state::surroundfi_group::InterestRateConfigOpt {
...
async fn add_too_many_arena_banks() -> anyhow::Result<()> {
    let test_f = TestFixture::new(None).await;
    let group_before = test_f.surroundfi_group.load().await;
    ...
    let res = test_f
        ...
        .await;
    assert!(res.is_ok());
    let group_after = test_f.surroundfi_group.load().await;
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::ArenaBankLimit);
    ...
    let res = test_f
        ...
        .await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::ArenaSettingCannotChange);
