...
use surroundfi::state::surroundfi_group::{Bank, BankConfig, BankVaultType, InterestRateConfig};
use pretty_assertions::assert_eq;
use solana_program_test::*;

#[tokio::test]
async fn surroundfi_group_accrue_interest_rates_success_1() -> anyhow::Result<()> {
...
#[tokio::test]
async fn surroundfi_group_accrue_interest_rates_success_2() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    // The program fee ata needs to exist, but doesn't need any assets.
    {
        ...
        let ata = TokenAccountFixture::new_from_ata(
            ...
            &test_f.surroundfi_group.fee_wallet,
    ...
    test_f.surroundfi_group.try_collect_fees(usdc_bank_f).await?;
