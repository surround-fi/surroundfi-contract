...
use surroundfi::{
    errors::SurroundfiError, state::surroundfi_account::ACCOUNT_TRANSFER_AUTHORITY_ALLOWED,
};
...
// RUST_BACKTRACE=1 cargo test-bpf surroundfi_account_authority_transfer_no_flag_set -- --exact
#[tokio::test]
async fn surroundfi_account_authority_transfer_no_flag_set() -> anyhow::Result<()> {
    let test_f = TestFixture::new(None).await;
    // Default account with no flags set
    let surroundfi_account = test_f.create_surroundfi_account().await;
    let new_authority = Keypair::new().pubkey();

    let res = surroundfi_account
        .try_transfer_account_authority(new_authority, None)
        .await;

    // Check transfer authority is unchanged
    let account = surroundfi_account.load().await;
    ...
    assert_custom_error!(
        res.unwrap_err(),
        SurroundfiError::IllegalAccountAuthorityTransfer
    );
...
#[tokio::test]
async fn surroundfi_account_authority_transfer_not_account_owner() -> anyhow::Result<()> {
    let test_f = TestFixture::new(None).await;
    let surroundfi_account = test_f.create_surroundfi_account().await;
    let new_authority = Keypair::new().pubkey();
    let signer = Keypair::new();

    let tx = surroundfi_account
