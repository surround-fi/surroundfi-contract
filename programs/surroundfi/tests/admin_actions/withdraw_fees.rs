...
#[tokio::test]
async fn surroundfi_group_withdraw_fees_and_insurance_fund_as_admin_success(
    bank_mint: BankMint,
) -> anyhow::Result<()> {
...
#[test_case(BankMint::T22WithFee)]
#[tokio::test]
async fn surroundfi_group_withdraw_fees_and_insurance_fund_as_non_admin_failure(
    bank_mint: BankMint,
) -> anyhow::Result<()> {
    // Setup test executor with non-admin payer
    let mut test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    // Update the admin of the surroundfi group
    test_f
        .surroundfi_group
