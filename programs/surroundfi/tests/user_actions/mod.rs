...
#[tokio::test]
async fn surroundfi_account_correct_balance_selection_after_closing_position() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    let mut surroundfi_account = lender_mfi_account_f.load().await;
...
async fn emissions_test() -> anyhow::Result<()> {
    ...
    assert_custom_error!(res.unwrap_err(), SurroundfiError::EmissionsAlreadySetup);

    // Fund SOL bank
    let sol_lender_account = test_f.create_surroundfi_account().await;
    ...
    let mfi_account_f = test_f.create_surroundfi_account().await;
...
async fn account_flags() -> anyhow::Result<()> {
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IllegalFlag);
