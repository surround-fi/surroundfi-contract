...
use surroundfi::{
    ...
    prelude::*,
    state::surroundfi_group::{BankConfigOpt, BankVaultType},
};
...
#[tokio::test]
async fn surroundfi_account_borrow_success(
    deposit_amount: f64,
    borrow_amount: f64,
    ...
    let post_vault_balance = debt_bank_f
        ...
        .balance()
        .await;
    let surroundfi_account = user_mfi_account_f.load().await;
    let balance = surroundfi_account
        .lending_account
    ...
    let active_balance_count = surroundfi_account
        .lending_account
    ...
    let health_cache = surroundfi_account.health_cache;
    ...
    for (i, bal) in surroundfi_account.lending_account.balances.iter().enumerate() {
...
#[tokio::test]
async fn surroundfi_account_borrow_failure_not_enough_collateral(
    deposit_amount: f64,
    ...
    let res = borrower_mfi_account_f
        ...
        .await;
    assert_custom_error!(res.unwrap_err(), SurroundfiError::RiskEngineInitRejected);
...
#[tokio::test]
async fn surroundfi_account_borrow_failure_borrow_limit(
    borrow_cap: f64,
    borrow_amount_ok: f64,
    borrow_amount_failed: f64,
    collateral_mint: BankMint,
    ...
    assert_custom_error!(
        res.unwrap_err(),
        SurroundfiError::BankLiabilityCapacityExceeded
    );
...
async fn isolated_borrows() -> anyhow::Result<()> {
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IsolatedAccountIllegalState);
