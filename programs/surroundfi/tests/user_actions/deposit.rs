...
use surroundfi::state::surroundfi_group::{BankConfigOpt, BankVaultType};
use surroundfi::{assert_eq_with_tolerance, prelude::*};
...
#[tokio::test]
async fn surroundfi_account_deposit_success(
    deposit_amount: f64,
    bank_mint: BankMint,
) -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
    ...
    if deposit_amount > 0. {
        ...
        let maybe_balance = surroundfi_account.lending_account.get_balance(&bank_f.key);
...
#[tokio::test]
async fn surroundfi_account_deposit_failure_capacity_exceeded(
    deposit_cap: f64,
    ...
    let res = user_mfi_account_f
        ...
        .await;
    assert_custom_error!(res.unwrap_err(), SurroundfiError::BankAssetCapacityExceeded);
...
#[tokio::test]
async fn surroundfi_account_deposit_failure_wrong_token_program() -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
    // Setup
    // -------------------------------------------------------------------------
    ...
    let accounts = surroundfi::accounts::LendingAccountDeposit {
        group: surroundfi_account.group,
        surroundfi_account: user_mfi_account_f.key,
...
#[tokio::test]
async fn surroundfi_account_deposit_up_to_limit_success(
    deposit_cap: f64,
    first_deposit: f64,
    second_deposit: f64,
    third_deposit: f64,
