...
use surroundfi::{
    prelude::*,
    state::surroundfi_group::{Bank, BankConfig, BankConfigOpt, BankVaultType},
};
...
#[tokio::test]
async fn surroundfi_account_liquidation_success(
    deposit_amount: f64,
    borrow_amount: f64,
    ...
    let (liquidatee_mfi_account_f, borrow_amount_actual) = {
        let liquidatee_mfi_account_f = test_f.create_surroundfi_account().await;
    ...
    let liquidator_mfi_account_f = {
        let liquidator_mfi_account_f = test_f.create_surroundfi_account().await;
...
#[tokio::test]
async fn surroundfi_account_liquidation_success_many_balances() -> anyhow::Result<()> {
...
#[tokio::test]
async fn surroundfi_account_liquidation_success_swb() -> anyhow::Result<()> {
...
#[tokio::test]
async fn surroundfi_account_liquidation_failure_liquidatee_not_unhealthy() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    assert_custom_error!(res.unwrap_err(), SurroundfiError::HealthyAccount);

    Ok(())
}

#[tokio::test]
async fn surroundfi_account_liquidation_failure_liquidation_too_severe() -> anyhow::Result<()> {
    ...
    assert_custom_error!(res.unwrap_err(), SurroundfiError::ExhaustedLiability);
...
#[tokio::test]
async fn surroundfi_account_liquidation_failure_liquidator_no_collateral() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    assert_custom_error!(res.unwrap_err(), SurroundfiError::OverliquidationAttempt);
...
#[tokio::test]
async fn surroundfi_account_liquidation_failure_bank_not_liquidatable() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
