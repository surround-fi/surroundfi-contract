...
use surroundfi::errors::SurroundfiError;
...
async fn lending_account_close_balance() -> anyhow::Result<()> {
    ...
    // Fund SOL lender
    let lender_sfi_account_f = test_f.create_surroundfi_account().await;
    let lender_token_account_sol = test_f
        .sol_equivalent_mint
        .create_token_account_and_mint_to(1_000)
        .await;
    lender_sfi_account_f
    ...
    let res = lender_sfi_account_f.try_balance_close(sol_bank).await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IllegalBalanceState);

    // Fund SOL borrower
    let borrower_sfi_account_f = test_f.create_surroundfi_account().await;
    ...
    let res = borrower_sfi_account_f.try_balance_close(sol_bank).await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IllegalBalanceState);
    ...
    let res = borrower_sfi_account_f
        ...
        .await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::NoLiabilityFound);

    // This issue is not that bad, because the user can still borrow other assets (isolated liab < empty threshold)
    let res = borrower_sfi_account_f.try_balance_close(sol_eq_bank).await;
    assert!(res.is_ok());
