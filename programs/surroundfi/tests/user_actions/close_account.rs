...
#[tokio::test]
async fn close_surroundfi_account() -> anyhow::Result<()> {
    ...
    let res = surroundfi_account_f.try_close_account(0).await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IllegalAction);

    let sol_bank_f = test_f.get_bank(&BankMint::Sol);
    let sol_account = test_f.sol_mint.create_token_account_and_mint_to(100).await;
    let depositor = test_f.create_surroundfi_account().await;
    depositor
    ...
    let res = surroundfi_account_f.try_close_account(1).await;
