...
#[tokio::test]
async fn surroundfi_account_repay_success(
    borrow_amount: f64,
    repay_amount: f64,
...
#[tokio::test]
async fn surroundfi_account_repay_all_success(
    borrow_amount: f64,
    collateral_mint: BankMint,
    debt_mint: BankMint,
) -> anyhow::Result<()> {
    ...
    let (post_vault_balance, post_accounted_vault_balance) = {
        let post_vault_balance = debt_bank
            ...
            .await;
        let surroundfi_account = user_mfi_account_f.load().await;
        let balance = surroundfi_account.lending_account.get_balance(&debt_bank.key);
...
#[tokio::test]
async fn surroundfi_account_repay_failure_repaying_too_much(
    borrow_amount: f64,
    repay_amount: f64,
    collateral_mint: BankMint,
    debt_mint: BankMint,
    ...
    let res = user_mfi_account_f
        ...
        .await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), SurroundfiError::OperationRepayOnly);
