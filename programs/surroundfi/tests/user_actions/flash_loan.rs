...
async fn flashloan_fail_account_health() -> anyhow::Result<()> {
    ...
    assert_custom_error!(
        ...
        SurroundfiError::RiskEngineInitRejected
    );
...
async fn flashloan_fail_missing_fe_ix() -> anyhow::Result<()> {
    ...
    assert_custom_error!(res.unwrap_err(), SurroundfiError::IllegalFlashloan);
...
async fn flashloan_fail_missing_invalid_sysvar_ixs() -> anyhow::Result<()> {
    ...
    let start_ix = Instruction {
        ...
        accounts: surroundfi::accounts::LendingAccountStartFlashloan {
            surroundfi_account: borrower_mfi_account_f.key,
            authority: test_f.context.borrow().payer.pubkey(),
            ixs_sysvar: Pubkey::default(),
        }
        .to_account_metas(Some(true)),
        data: surroundfi::instruction::LendingAccountStartFlashloan {
