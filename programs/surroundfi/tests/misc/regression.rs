...
use surroundfi::{
    ...
    state::{
        ...
        surroundfi_group::{Bank, BankOperationalState, RiskTier},
        price::OracleSetup,
    },
};
...
async fn account_field_values_reg() -> anyhow::Result<()> {
    let account_fixtures_path = "tests/fixtures/surroundfi_account";

    // Sample 1

    let mut path = PathBuf::from_str(account_fixtures_path).unwrap();
    path.push("surroundfi_account_sample_1.json");
    ...
    let account = SurroundfiAccount::try_deserialize(&mut BASE64_STANDARD.decode(data)?.as_slice())?;
    ...
    let mut path = PathBuf::from_str(account_fixtures_path).unwrap();
    path.push("surroundfi_account_sample_2.json");
    let mut file = File::open(&path).unwrap();
    let mut account_info_raw = String::new();
    file.read_to_string(&mut account_info_raw).unwrap();
    ...
    let mut path = PathBuf::from_str(account_fixtures_path).unwrap();
    path.push("surroundfi_account_sample_3.json");
    let mut file = File::open(&path).unwrap();
    let mut account_info_raw = String::new();
    file.read_to_string(&mut account_info_raw).unwrap();
