...
use surroundfi::{
    constants::TOTAL_ASSET_VALUE_INIT_LIMIT_INACTIVE, prelude::SurroundfiError,
    state::surroundfi_group::BankConfigOpt,
};
use pretty_assertions::assert_eq;
use solana_program_test::*;

#[tokio::test]
async fn surroundfi_group_init_limit_0() -> anyhow::Result<()> {
    ...
    let sol_depositor = test_f.create_surroundfi_account().await;
    let usdc_depositor = test_f.create_surroundfi_account().await;
