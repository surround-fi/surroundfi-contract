...
use surroundfi::{
    ...
    state::surroundfi_group::BankVaultType,
};
...
impl AccountsState {
    ...
    pub fn new_surroundfi_program(&self) -> AccountInfo {
        self.new_program(surroundfi::id())
    }
...
pub fn get_vault_address(bank: &Pubkey, vault_type: BankVaultType) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[vault_type.get_seed(), &bank.to_bytes()], &surroundfi::ID)
}
