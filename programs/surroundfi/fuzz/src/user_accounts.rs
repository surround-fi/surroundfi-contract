...
impl<'info> UserAccount<'info> {
    ...
    pub fn get_liquidation_banks(
        ...
        let surroundfi_account_al =
            AccountLoader::<SurroundfiAccount>::try_from(&self.margin_account).ok()?;
        let surroundfi_account = surroundfi_account_al.load().ok()?;

        // let bank_map = get_bank_map(banks);

        let mut asset_balances = surroundfi_account
            .lending_account
            .balances
            .iter()
            .filter(|blc| !blc.is_empty(surroundfi::state::surroundfi_account::BalanceSide::Assets))
            .collect::<Vec<_>>();
        let mut liab_balances = surroundfi_account
            .lending_account
            .balances
            .iter()
            .filter(|blc| {
                !blc.is_empty(surroundfi::state::surroundfi_account::BalanceSide::Liabilities)
            })
            .collect::<Vec<_>>();
    ...
    pub fn get_remaining_accounts(
        ...
        let surroundfi_account_al =
            AccountLoader::<SurroundfiAccount>::try_from(&self.margin_account).unwrap();
        let surroundfi_account = surroundfi_account_al.load().unwrap();

        let mut already_included_banks = HashSet::new();

        let mut ais = surroundfi_account
            .lending_account
            .balances
            .iter()
