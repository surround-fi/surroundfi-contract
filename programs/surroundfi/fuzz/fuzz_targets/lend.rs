...
use surroundfi::{assert_eq_with_tolerance, prelude::SurroundfiGroup, state::surroundfi_group::Bank};
use surroundfi_fuzz::{
    account_state::AccountsState, arbitrary_helpers::*, metrics::Metrics, SurroundfiFuzzContext,
};
...
fn process_actions(ctx: FuzzerContext) -> Result<()> {
    ...
    let mut context =
        SurroundfiFuzzContext::setup(&accounst_state, &ctx.initial_bank_configs, N_USERS as u8);
...
fn verify_end_state<'a>(mga: &'a SurroundfiFuzzContext<'a>) -> anyhow::Result<()> {
    let group = AccountLoader::<SurroundfiGroup>::try_from(&mga.surroundfi_group).unwrap();
    mga.banks.iter().try_for_each(|bank| {
        ...
        surroundfi_fuzz::log!("Accounted Deposits: {}, Liabs: {}, Net {}, Outstanding Fees: {}, Net with Fees {}, Value Token Balance {}, Net Without Fees {}",
            total_deposits,
            total_liabilities,
...
fn process_action<'bump>(action: &Action, mga: &'bump SurroundfiFuzzContext<'bump>) -> Result<()> {
    surroundfi_fuzz::log!("==================>Action {:?}", action);
    match action {
        Action::Deposit {
            account,
            bank,
