...
use {
    ...
    surroundfi::{
        ...
        prelude::*,
        state::{
            surroundfi_account::{BankAccountWrapper, SurroundfiAccount},
            surroundfi_group::{
...
pub fn group_get(config: Config, surroundfi_group: Option<Pubkey>) -> Result<()> {
    if let Some(surroundfi_group) = surroundfi_group {
        println!("Address: {surroundfi_group}");
        println!("=============");
        print_group_banks(config, surroundfi_group)?;
    } else {
...
pub fn group_get_all(config: Config) -> Result<()> {
    let accounts: Vec<(Pubkey, SurroundfiGroup)> = config.sfi_program.accounts(vec![])?;
...
fn print_group(address: &Pubkey, group: &SurroundfiGroup) {
    println!(
        r#"
Group: {}
Admin: {}
...
pub fn print_group_banks(config: Config, surroundfi_group: Pubkey) -> Result<()> {
    let banks = config
        .sfi_program
        .accounts::<Bank>(vec![RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            8 + size_of::<Pubkey>() + size_of::<u8>(),
            surroundfi_group.to_bytes().to_vec(),
        ))])?;
...
pub fn group_create(
    ...
    if profile.surroundfi_group.is_some() && !override_existing_profile_group {
        bail!(
            "Surroundfi group already exists for profile [{}]",
            profile.name
        );
    }

    let surroundfi_group_keypair = Keypair::new();

    let init_surroundfi_group_ixs_builder = config.sfi_program.request();

    let mut signing_keypairs = config.get_signers(false);
    signing_keypairs.push(&surroundfi_group_keypair);

    let init_surroundfi_group_ixs = init_surroundfi_group_ixs_builder
        .accounts(surroundfi::accounts::SurroundfiGroupInitialize {
            surroundfi_group: surroundfi_group_keypair.pubkey(),
            admin,
            fee_state: find_fee_state_pda(&surroundfi::id()).0,
            system_program: system_program::id(),
        })
        .args(surroundfi::instruction::SurroundfiGroupInitialize { is_arena_group })
        .instructions()?;

    let recent_blockhash = rpc_client.get_latest_blockhash().unwrap();
    let message = Message::new(&init_surroundfi_group_ixs, Some(&config.authority()));
    ...
    match process_transaction(&transaction, &rpc_client, config.get_tx_mode()) {
        Ok(sig) => println!("surroundfi group created (sig: {})", sig),
        Err(err) => {
            println!("Error during surroundfi group creation:\n{:#?}", err);
            return Err(anyhow!("Error during surroundfi group creation"));
        }
    };

    let mut profile = profile;
    profile.set_surroundfi_group(surroundfi_group_keypair.pubkey())?;
...
pub fn group_configure(
    ...
    if profile.surroundfi_group.is_none() {
        bail!("Surroundfi group not specified in profile [{}]", profile.name);
    }

    let signing_keypairs = config.get_signers(false);
    let configure_surroundfi_group_ixs_builder = config
        .sfi_program
        .request()
        .signer(*signing_keypairs.first().unwrap());

    let configure_surroundfi_group_ixs = configure_surroundfi_group_ixs_builder
        .accounts(surroundfi::accounts::SurroundfiGroupConfigure {
            surroundfi_group: profile.surroundfi_group.unwrap(),
            admin: config.authority(),
        })
        .args(surroundfi::instruction::SurroundfiGroupConfigure {
            new_admin,
            is_arena_group,
        })
        .instructions()?;
    ...
    let recent_blockhash = rpc_client.get_latest_blockhash().unwrap();
    let message = Message::new(&configure_surroundfi_group_ixs, Some(&config.authority()));
    let mut transaction = Transaction::new_unsigned(message);
    transaction.partial_sign(&signing_keypairs, recent_blockhash);

    match process_transaction(&transaction, &rpc_client, config.get_tx_mode()) {
        Ok(sig) => println!("surroundfi group created (sig: {})", sig),
        Err(err) => println!("Error during surroundfi group creation:\n{:#?}", err),
    };
...
fn create_bank_ix_with_seed(
    ...
    let group_key = profile.surroundfi_group.unwrap();
    ...
    let add_bank_ixs_builder = config.sfi_program.request();
    let add_bank_ixs = add_bank_ixs_builder
        .accounts(surroundfi::accounts::LendingPoolAddBankWithSeed {
            surroundfi_group: profile.surroundfi_group.unwrap(),
            admin: config.authority(),
            bank_mint,
            bank: bank_pda,
        ...
        .args(surroundfi::instruction::LendingPoolAddBankWithSeed {
...
fn create_bank_ix(
    ...
    let add_bank_ixs_builder = config.sfi_program.request();
    let add_bank_ixs = add_bank_ixs_builder
        .accounts(surroundfi::accounts::LendingPoolAddBank {
            surroundfi_group: profile.surroundfi_group.unwrap(),
            admin: config.authority(),
        ...
        .args(surroundfi::instruction::LendingPoolAddBank {
            bank_config: BankConfigCompact {
                asset_weight_init,
                asset_weight_maint,
                liability_weight_init,
...
pub fn group_handle_bankruptcy(
    ...
    let banks = HashMap::from_iter(load_all_banks(
        config,
        Some(profile.surroundfi_group.unwrap()),
    )?);

    let surroundfi_account = config
        .sfi_program
        .account::<SurroundfiAccount>(surroundfi_account_pk)?;
...
fn handle_bankruptcy_for_an_account(
    ...
    let mut handle_bankruptcy_ix = Instruction {
        ...
        accounts: surroundfi::accounts::LendingPoolHandleBankruptcy {
            group: profile.surroundfi_group.unwrap(),
            signer: config.authority(),
            bank: bank_pk,
            surroundfi_account: surroundfi_account_pk,
            liquidity_vault: find_bank_vault_pda(
                &bank_pk,
                BankVaultType::Liquidity,
                &config.program_id,
...
pub fn handle_bankruptcy_for_accounts(
    ...
    for account in accounts {
        let surroundfi_account = config
            .sfi_program
            .account::<SurroundfiAccount>(account)
            .unwrap();
...
pub fn initialize_fee_state(
    ...
    let initialize_fee_state_ixs_builder = config.sfi_program.request();
    ...
    let initialize_fee_state_ixs = initialize_fee_state_ixs_builder
        ...
        .args(surroundfi::instruction::InitGlobalFeeState {
            admin,
            fee_wallet,
...
pub fn edit_fee_state(
    ...
    let edit_fee_state_ixs_builder = config.sfi_program.request();

    let edit_fee_state_ixs = edit_fee_state_ixs_builder
        .accounts(surroundfi::accounts::EditFeeState {
            global_fee_admin: config.authority(),
            fee_state: fee_state_pubkey,
        })
        .args(surroundfi::instruction::EditGlobalFeeState {
            admin: new_admin,
            fee_wallet,
            bank_init_flat_sol_fee,
            program_fee_fixed,
...
pub fn config_group_fee(config: Config, profile: Profile, enable_program_fee: bool) -> Result<()> {
    let rpc_client = config.sfi_program.rpc();
    let surroundfi_group_pubkey = profile.surroundfi_group.ok_or_else(|| {
        anyhow!(
            "Surroundfi group does not exist for profile [{}]",
            profile.name
        )
    })?;
    ...
    let config_group_fee_ixs_builder = config.sfi_program.request();

    let config_group_fee_ixs = config_group_fee_ixs_builder
        .accounts(surroundfi::accounts::ConfigGroupFee {
            surroundfi_group: surroundfi_group_pubkey,
            global_fee_admin: config.authority(),
            fee_state: fee_state_pubkey,
        })
        .args(surroundfi::instruction::ConfigGroupFee { enable_program_fee })
        .instructions()?;
...
pub fn propagate_fee(config: Config, surroundfi_group: Pubkey) -> Result<()> {
    let rpc_client = config.sfi_program.rpc();

    let fee_state_pubkey = find_fee_state_pda(&config.program_id).0;

    let propagate_fee_ixs_builder = config.sfi_program.request();

    let propagate_fee_ixs = propagate_fee_ixs_builder
        .accounts(surroundfi::accounts::PropagateFee {
            fee_state: fee_state_pubkey,
            surroundfi_group,
        })
        .args(surroundfi::instruction::PropagateFeeState {})
        .instructions()?;
...
pub fn bank_get(config: Config, bank_pk: Option<Pubkey>) -> Result<()> {
    ...
    if let Some(address) = bank_pk {
        let mut bank: Bank = config.sfi_program.account(address)?;
        let group: SurroundfiGroup = config.sfi_program.account(bank.group)?;
...
fn load_all_banks(config: &Config, surroundfi_group: Option<Pubkey>) -> Result<Vec<(Pubkey, Bank)>> {
    info!("Loading banks for group {:?}", surroundfi_group);
    let filters = match surroundfi_group {
        Some(surroundfi_group) => vec![RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            8 + size_of::<Pubkey>() + size_of::<u8>(),
            surroundfi_group.to_bytes().to_vec(),
        ))],
        None => vec![],
    ...
    let banks_with_addresses = config.sfi_program.accounts::<Bank>(filters)?;

    Ok(banks_with_addresses)
}

pub fn bank_get_all(config: Config, surroundfi_group: Option<Pubkey>) -> Result<()> {
    let accounts = load_all_banks(&config, surroundfi_group)?;
...
pub fn bank_inspect_price_oracle(config: Config, bank_pk: Pubkey) -> Result<()> {
    use surroundfi::state::price::{OraclePriceType, PriceBias};

    let bank: Bank = config.sfi_program.account(bank_pk)?;
...
pub fn bank_setup_emissions(
    ...
    let emissions_mint_account = config.sfi_program.rpc().get_account(&mint).unwrap();
    ...
    let ix = Instruction {
        ...
        accounts: surroundfi::accounts::LendingPoolSetupEmissions {
            group: profile.surroundfi_group.expect("surroundfi group not set"),
            admin: config.authority(),
            bank,
            emissions_mint: mint,
...
pub fn bank_configure(
    ...
    let configure_bank_ixs_builder = config.sfi_program.request();
    let signing_keypairs = config.get_signers(false);

    let configure_bank_ixs = configure_bank_ixs_builder
        .accounts(surroundfi::accounts::LendingPoolConfigureBank {
            group: profile.surroundfi_group.unwrap(),
            admin: config.authority(),
            bank: bank_pk,
        })
        .args(surroundfi::instruction::LendingPoolConfigureBank {
...
pub fn bank_configure_oracle(
    ...
    let mut configure_bank_ixs = configure_bank_ixs_builder
        .accounts(surroundfi::accounts::LendingPoolConfigureBankOracle {
            group: profile.surroundfi_group.unwrap(),
            admin: config.authority(),
            bank: bank_pk,
        })
        .args(surroundfi::instruction::LendingPoolConfigureBankOracle {
            setup,
...
pub fn create_profile(
    ...
    surroundfi_account: Option<Pubkey>,
) -> Result<()> {
...
pub fn surroundfi_account_list(profile: Profile, config: &Config) -> Result<()> {
    let group = profile.surroundfi_group.expect("Missing surroundfi group");
    let authority = config.authority();

    let banks = HashMap::from_iter(load_all_banks(config, Some(group))?);

    let accounts = config.sfi_program.accounts::<SurroundfiAccount>(vec![
    ...
    if accounts.is_empty() {
        println!("No surroundfi accounts found");
    }

    for (address, surroundfi_account) in accounts {
        print_account(
            address,
            surroundfi_account,
            banks.clone(),
    ...
    surroundfi_account
        ...
        .for_each(|balance| {
            ...
            let balance_amount = if balance
                .is_empty(surroundfi::state::surroundfi_account::BalanceSide::Assets)
                .not()
            {
                ...
                .is_empty(surroundfi::state::surroundfi_account::BalanceSide::Liabilities)
                .not()
            {
...
pub fn surroundfi_account_use(
    ...
    if surroundfi_account.group != group {
        return Err(anyhow!("Surroundfi account does not belong to group"));
    }

    if surroundfi_account.authority != authority {
        return Err(anyhow!("Surroundfi account does not belong to authority"));
    }
    ...
    profile.config(
        ...
        None,
        None,
        Some(surroundfi_account_pk),
    )?;

    println!("Default surroundfi account set to: {surroundfi_account_pk}");

    Ok(())
}

/// Print the surroundfi account for the provided address or the default surroundfi account if none is provided
///
/// If surroundfi account address is provided use the group in the surroundfi account data, otherwise use the profile defaults
pub fn surroundfi_account_get(
    profile: Profile,
    config: &Config,
    surroundfi_account_pk: Option<Pubkey>,
) -> Result<()> {
    let surroundfi_account_pk =
        surroundfi_account_pk.unwrap_or_else(|| profile.surroundfi_account.unwrap());

    let surroundfi_account = config
        .sfi_program
        .account::<SurroundfiAccount>(surroundfi_account_pk)?;

    let group = surroundfi_account.group;

    let banks = HashMap::from_iter(load_all_banks(config, Some(group))?);

    print_account(surroundfi_account_pk, surroundfi_account, banks, false)?;

    Ok(())
}

pub fn surroundfi_account_deposit(
    profile: &Profile,
    config: &Config,
    bank_pk: Pubkey,
    ui_amount: f64,
    ...
    let surroundfi_account_pk = profile.get_surroundfi_account();
    ...
    if bank.group != profile.surroundfi_group.unwrap() {
    ...
    let mut ix = Instruction {
        program_id: config.program_id,
        accounts: surroundfi::accounts::LendingAccountDeposit {
            group: profile.surroundfi_group.unwrap(),
            surroundfi_account: surroundfi_account_pk,
...
pub fn surroundfi_account_withdraw(
    profile: &Profile,
    config: &Config,
    bank_pk: Pubkey,
    ui_amount: f64,
    ...
    let mut ix = Instruction {
        program_id: config.program_id,
        accounts: surroundfi::accounts::LendingAccountWithdraw {
            group: profile.surroundfi_group.unwrap(),
            surroundfi_account: surroundfi_account_pk,
            authority: signer.pubkey(),
            bank: bank_pk,
...
pub fn surroundfi_account_borrow(
    profile: &Profile,
    config: &Config,
    bank_pk: Pubkey,
    ui_amount: f64,
    ...
    let mut ix = Instruction {
        program_id: config.program_id,
        accounts: surroundfi::accounts::LendingAccountBorrow {
            group: profile.surroundfi_group.unwrap(),
            surroundfi_account: surroundfi_account_pk,
            authority: signer.pubkey(),
            bank: bank_pk,
        ...
        .to_account_metas(Some(true)),
        data: surroundfi::instruction::LendingAccountBorrow { amount }.data(),
    };
...
pub fn surroundfi_account_liquidate(
    profile: &Profile,
    config: &Config,
    liquidatee_surroundfi_account_pk: Pubkey,
    ...
    let liquidatee_surroundfi_account = config
        .sfi_program
        .account::<SurroundfiAccount>(liquidatee_surroundfi_account_pk)?;
    ...
    if asset_bank.group != profile.surroundfi_group.unwrap() {
        bail!("Asset bank does not belong to group")
    }
    if liability_bank.group != profile.surroundfi_group.unwrap() {
    ...
    let mut ix = Instruction {
        program_id: config.program_id,
        accounts: surroundfi::accounts::LendingAccountLiquidate {
            group: profile.surroundfi_group.unwrap(),
            asset_bank: asset_bank_pk,
            liab_bank: liability_bank_pk,
            liquidator_surroundfi_account: surroundfi_account_pk,
            authority: signer.pubkey(),
        ...
        .to_account_metas(Some(true)),
        data: surroundfi::instruction::LendingAccountLiquidate { asset_amount }.data(),
    };
    ...
    match process_transaction(&tx, &config.sfi_program.rpc(), config.get_tx_mode()) {
...
pub fn surroundfi_account_create(profile: &Profile, config: &Config) -> Result<()> {
    let signer = config.get_non_ms_authority_keypair()?;
    ...
    let ix = Instruction {
        program_id: config.program_id,
        accounts: surroundfi::accounts::SurroundfiAccountInitialize {
            surroundfi_group: profile.surroundfi_group.unwrap(),
    ...
    let tx = Transaction::new_signed_with_payer(
        &[ix],
        Some(&signer.pubkey()),
        &[signer, &surroundfi_account_key],
        recent_blockhash,
    );

    let surroundfi_account_pk = surroundfi_account_key.pubkey();

    match process_transaction(&tx, &config.sfi_program.rpc(), config.get_tx_mode()) {
        Ok(_sig) => print!("{surroundfi_account_pk}"),
    ...
    profile.config(
        ...
        None,
        None,
        Some(surroundfi_account_key.pubkey()),
    )?;

    Ok(())
}

pub fn surroundfi_account_close(profile: &Profile, config: &Config) -> Result<()> {
    let signer = config.get_non_ms_authority_keypair()?;

    let rpc_client = config.sfi_program.rpc();

    let surroundfi_account_pk = profile.get_surroundfi_account();
    println!("Closing surroundfi account {}", surroundfi_account_pk);
    ...
    match process_transaction(&tx, &config.sfi_program.rpc(), config.get_tx_mode()) {
        Ok(sig) => println!("Surroundfi account closed successfully (sig: {})", sig),
        Err(err) => println!("Error during surroundfi account closure:\n{:#?}", err),
    };
...
pub fn process_list_lip_campaigns(config: &Config) {
    ...
    campaings.iter().for_each(|(address, campaign)| {
        let bank = config
            .sfi_program
            .account::<Bank>(campaign.surroundfi_bank_pk)
            .unwrap();
...
pub fn process_list_deposits(config: &Config) {
    ...
    let banks =
        HashMap::<Pubkey, Bank>::from_iter(config.sfi_program.accounts::<Bank>(vec![]).unwrap());
    ...
    deposits.iter().for_each(|(address, deposit)| {
        ...
        let bank = banks.get(&campaign.surroundfi_bank_pk).unwrap();
