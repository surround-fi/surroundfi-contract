...
pub fn find_pyth_push_oracles_for_feed_id(
    ...
    for (ref address, account) in res.iter_mut() {
        ...
        let price_update_v2 = surroundfi::state::price::load_price_update_v2_checked(&ai)?;
...
pub fn inspect_pyth_push_feed(config: &Config, address: Pubkey) -> anyhow::Result<()> {
    let mut account = config.sfi_program.rpc().get_account(&address)?;
    ...
    println!(
        "Price: {}",
        feed.get_price_of_type(surroundfi::state::price::OraclePriceType::RealTime, None)?
    );
