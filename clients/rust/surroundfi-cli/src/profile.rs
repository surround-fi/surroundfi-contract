...
pub struct Profile {
    ...
    pub surroundfi_group: Option<Pubkey>,
    pub surroundfi_account: Option<Pubkey>,
}
...
impl Profile {
    ...
    pub fn config(
        ...
        if let Some(group) = group {
            self.surroundfi_group = Some(group);
        }

        if let Some(account) = account {
            self.surroundfi_account = Some(account);
        }
    ...
    pub fn get_surroundfi_account(&self) -> Pubkey {
        self.surroundfi_account
            .unwrap_or_else(|| panic!("No surroundfi account set for profile \"{}\"", self.name))
    }

    pub fn set_surroundfi_group(&mut self, address: Pubkey) -> Result<()> {
        self.surroundfi_group = Some(address);
        self.write_to_file()?;
